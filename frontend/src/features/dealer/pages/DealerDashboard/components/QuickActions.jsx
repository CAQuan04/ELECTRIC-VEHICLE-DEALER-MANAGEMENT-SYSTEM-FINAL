...
const QuickActions = ({ navigate }) => {
  ...
  const staffActions = [
    { icon: FileText, label: 'Tạo báo giá', color: 'emerald', path: dealerId ? `/${dealerId}/dealer/quotations/create` : '/dealer/quotations/create' },
    { icon: Calendar, label: 'Đặt lịch lái thử', color: 'blue', path: dealerId ? `/${dealerId}/dealer/test-drives/new` : '/dealer/test-drives/new' },
    { icon: ShoppingCart, label: 'Tạo đơn hàng', color: 'purple', path: dealerId ? `/${dealerId}/dealer/orders/create` : '/dealer/orders/create' },
    { icon: UserPlus, label: 'Thêm khách hàng', color: 'pink', path: dealerId ? `/${dealerId}/dealer/customers/new` : '/dealer/customers/new' }
  ];

  // Manager-only actions - advanced operations
  const managerActions = [
    { icon: Settings, label: 'Quản lý nhân viên', color: 'indigo', path: dealerId ? `/${dealerId}/dealer/staff` : '/dealer/staff' },
    { icon: TrendingUp, label: 'Phân tích doanh thu', color: 'orange', path: dealerId ? `/${dealerId}/dealer/reports/sales-performance` : '/dealer/reports/sales-performance' },
    { icon: Shield, label: 'Phê duyệt đơn hàng', color: 'red', path: dealerId ? `/${dealerId}/dealer/orders` : '/dealer/orders' },
    { icon: Package, label: 'Quản lý kho', color: 'teal', path: dealerId ? `/${dealerId}/dealer/inventory` : '/dealer/inventory' }
  ];
