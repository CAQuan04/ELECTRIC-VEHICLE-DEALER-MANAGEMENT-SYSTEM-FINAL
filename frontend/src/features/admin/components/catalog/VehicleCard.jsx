...
const VehicleCard = ({
  ...
  return (
    <div className="bg-[#13233a] rounded-2xl border border-slate-700 shadow-lg hover:shadow-cyan-900/40 transition p-5">
      {/* Ảnh xe */}
      ...
      <p className="text-slate-300 font-medium mt-1">{vehicle.basePrice} VNĐ</p>
      <span
            ...
            {vehicle.configs.map((cfg) => (
              <div
                ...
                className={`rounded-xl p-2 border ${
                  cfg.status === "Active"
                    ? "bg-[#1a2b44] border-slate-700"
                    : "bg-[#1a1f2e] border-slate-800 opacity-75"
                } transition`}
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => onToggleConfigStatus(cfg)}
                      className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-lg transition ${
                        cfg.status === "Active"
                ...
                <div className="text-slate-400 text-xs mt-1">
                  🔋 {cfg.battery} • 🚗 {cfg.range} km
                </div>
                {cfg.colorOptions?.length > 0 && (
                  <div className="text-xs text-slate-500 mt-1">
                    🎨 {cfg.colorOptions.join(", ")}
                  </div>
                )}
      ...
      <div className="flex justify-between mt-4">
        {vehicle.status === "Active" ? (
          ...
          <button
            ...
            className="flex items-center gap-1 text-emerald-400 hover:text-emerald-300 text-sm mx-auto"
          >
