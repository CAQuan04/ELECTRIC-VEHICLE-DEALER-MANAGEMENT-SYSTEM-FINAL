...
const MultiStepRegister = () => {
  ...
  // Memoize location state to prevent infinite re-renders
  const locationState = useMemo(() => ({
    initialData: location.state?.initialData || {},
    editMode: location.state?.editMode || false,
    targetStep: location.state?.targetStep || STEPS.BASIC_INFO,
    returnToSuccess: location.state?.returnToSuccess || false
  }), [location.state]);

  const [currentStep, setCurrentStep] = useState(locationState.editMode ? locationState.targetStep : STEPS.BASIC_INFO);
  const [formData, setFormData] = useState({
    // Basic info
    email: locationState.initialData.email || '',
    password: locationState.initialData.password || '',
    confirmPassword: '',
    fullName: locationState.initialData.fullName || locationState.initialData.username || '',
    role: 'customer',
    
    // Personal info
    phone: locationState.initialData.phone || '',
    dateOfBirth: locationState.initialData.dateOfBirth || '',
    gender: locationState.initialData.gender || '',
    address: locationState.initialData.address || '',
    provinceId: locationState.initialData.provinceId || '',
    provinceName: locationState.initialData.provinceName || '',
    wardId: locationState.initialData.wardId || '',
    wardName: locationState.initialData.wardName || '',
    
    // Survey info
    interestedVehicles: locationState.initialData.interestedVehicles || [],
    budgetRange: locationState.initialData.budgetRange || '',
    purchaseTimeframe: locationState.initialData.purchaseTimeframe || '',
    
    // System fields
    tempUserId: locationState.initialData.tempUserId || null,
    emailVerified: false,
